...
c = conn.cursor()
c.execute(
    """CREATE TABLE IF NOT EXISTS users (discord_id INTEGER, minecraft_username TEXT, PRIMARY KEY (discord_id, minecraft_username))"""
)
conn.commit()
...
async def add_user_to_database(discord_id: int, minecraft_username: str):
    c.execute(
        "INSERT OR IGNORE INTO users (discord_id, minecraft_username) VALUES (?, ?)",
...
async def remove_user_from_database(discord_id: int, minecraft_username: str):
    c.execute(
        "DELETE FROM users WHERE discord_id=? AND minecraft_username=?",
        (discord_id, minecraft_username),
    )
    conn.commit()
    await update_presence()
...
async def minecraft_username_exists_in_database(minecraft_username: str):
    c.execute("SELECT * FROM users WHERE minecraft_username=?",
              (minecraft_username,))
    if c.fetchone() is not None:
        return True
    else:
...
async def update_presence():
    ...
    await bot.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.watching, name=f"{count} users in the whitelist"
        )
    )
    ...
    try:
        ...
        with MCRcon(RCON_IP, RCON_PASSWORD, RCON_PORT) as mcr:
            response = await loop.run_in_executor(
                None, mcr.command, f"whitelist add {username}"
            )
        return response
...
async def remove_user_from_whitelist(minecraft_username: str):
    try:
        ...
        with MCRcon(
            ...
            response = await loop.run_in_executor(
                None, mcr.command, f"whitelist remove {minecraft_username}"
            )
        return response
    except Exception as e:
        print(e)
...
async def process_remove_user(
    ...
    if rcon_response is not None:
        ...
        await message.channel.send(
            f"An error occurred while removing `{minecraft_username}` from the whitelist.",
            reference=message,
        )
...
async def on_message(message):
    ...
    if (
        re.match(r"^[a-zA-Z0-9_]{3,16}$", message.content)
        and (not await user_has_added_to_database(message.author.id) or has_role)
        and not await minecraft_username_exists_in_database(message.content.strip())
    ):
        ...
        if is_premium:
            ...
            if rcon_response is not None:
                ...
                await message.channel.send(
                    f"An error occurred while adding `{minecraft_username}` to the whitelist.",
                    reference=message,
                )
                do_delete = False

    # Check if the message is a remove user command with a mention or a
    # username
    remove_command = re.search(
        r"^!((<@!?(\d+)>)|([a-zA-Z0-9_]{3,16}))$", message.content
    )
    if remove_command:
        ...
        if mentioned_user:
            if message.author.id == int(mentioned_user) or has_role:
                c.execute(
                    "SELECT minecraft_username FROM users WHERE discord_id=?",
                    (int(mentioned_user),),
                )
                usernames = c.fetchall()
                for username in usernames:
                    do_delete = False
                    await process_remove_user(
                        message, username[0], discord_user_id=int(mentioned_user)
                    )

        elif minecraft_username:
            c.execute(
                "SELECT discord_id FROM users WHERE minecraft_username=?",
                (minecraft_username,),
            )
            ...
            if has_role and user_data:
                do_delete = False
                await process_remove_user(
                    message, minecraft_username, discord_user_id=user_data[0]
                )
            elif user_data and user_data[0] == message.author.id:
                do_delete = False
                await process_remove_user(
                    message, minecraft_username, discord_user_id=message.author.id
                )
            elif not user_data:
                do_delete = False
                await message.channel.send(
                    f"`{minecraft_username}` is not in the whitelist.",
                    reference=message,
                )
